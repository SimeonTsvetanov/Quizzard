...
export const TeamCard: React.FC<TeamCardProps> = ({
   ...
   * Ensures input always reflects current round score.
   * Clears any validation errors when data changes.
   */
  useEffect(() => {
    setInputValue(currentRoundScore.toString());
    setHasError(false);
   ...
   * Input Blur Handler
   * 
   * Ensures input field always has a valid value when user leaves it.
   * Resets to current round score if input is empty or has errors.
   * Provides consistent user experience and prevents invalid states.
   */
  const handleBlur = () => {
    if (inputValue === '' || hasError) {
      // Reset to current round score if input is invalid
      setInputValue(currentRoundScore.toString());
      setHasError(false);
    }
  };
  ...
  return (
    <Card
      ...
      sx={{
        ...
        border: '1px solid',
        borderColor: isHovered || isFocused ? 'primary.main' : 'rgba(0,0,0,0.06)',
