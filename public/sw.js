...
self.addEventListener('fetch', event => {
  ...
  const isDirectFileRequest = url.pathname.match(/\.(js|css|png|jpg|jpeg|gif|svg|ico|json)$/i);
  
  if (isNavigationRequest && isHtmlRequest && isOurDomain && isQuizzardPath && !isDirectFileRequest) {
    // For SPA navigation requests, serve the index.html
    event.respondWith(
      caches.match('/Quizzard/index.html')
        .then(cachedResponse => {
          if (cachedResponse) {
